...
- **Структура раунда:**  
  Каждый раунд (за исключением тридцать второго, где обмен местами не производится) строится по принципу сети Фейстеля и включает следующие шаги:
  ...
  3. **Циклический сдвиг:** Результат нелинейного преобразования циклически сдвигается влево на 11 разрядов. При этом выбывающие биты выделяются и попадают в правую часть числа.
  4. **Операция XOR:** Полученное значение XOR-ится с левой частью блока.
...
- **Классы `Magma` и `MagmaCBC`:**  
  - Реализуют базовые операции алгоритма «Магма»: подготовку раундовых ключей, работу S-box, циклический сдвиг, разделение блока, а также функции для зашифрования и расшифрования 64-битных блоков.
  - В проекте класс `MagmaCBC` наследует базовый класс `Magma` и переопределяет методы `encrypt` и `decrypt` таким образом, чтобы учитывать XOR с предыдущим блоком при использовании вектора инициализации (IV).
...
- **Обработка блоков**: Данные разбиваются на 64-битные блоки. Методы `_encrypt_block` и `_decrypt_block` выполняют базовые операции зашифрования/расшифрования, а методы `encrypt` и `decrypt` управляют потоком данных, добавляя/удаляя дополнение.
- **Поддержка форматов**:  
...
1. Выбрать операцию (зашифрование/расшифрование)  
2. Указать 256-битный ключ в hex-формате  
3. Выбрать режим (ECB/CBC)  
4. Для CBC: ввести или сгенерировать 64-битный вектор инициализации IV  
...
- **Валидация**: Проверка на этапе ввода длины ключа шифрования (64 hex-символа) и вектора инициализации IV (16 hex-символов).
...
- Для режима **CBC** вектор инициализации (IV) должен быть одинаковым при зашифровании и расшифровании.
- В программе используется обработка ошибок, например, для неверного формата ключа или данных.
- Программа поддерживает зашифрование и расшифрование как текстовых строк, так и файлов с выводом в консоль или в файл.

---

## Модуль PKCS#7 Padding

Модуль `PKCS7Padding` реализует механизм дополнения (padding) данных для блочных шифров, чтобы длина исходного сообщения была кратна размеру блока.
...
#### Пример 2.1. Зашифрование текста в режиме ECB, вывод в консоль
...
#### Пример 2.2. Зашифрование текста в режиме CBC с вектором инициализации, вывод в файл
...
- Для всех команд параметры `-e/-d` (операция шифрования) и `-k` (ключ шифрования) обязательны.
- Параметр операции шифрования, если не указан, будет выбран ECB (`-m ecb`)
